...
class RevisedSyscallPerformanceReward(AbstractReward):
    ...
    def compute_reward(self, syscall_features, action, done):
        ...
        if anomalous:
            ...
            logging.debug(f"REWARD: hid {rate} {math.log(rate+1)} {self.r_hidden}")
            reward = math.log(rate + 1) + abs(self.r_hidden)  # ln(r + 1) + h
